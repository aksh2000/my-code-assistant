...
data class ChatMessage(val content: String, val role: String = "user") {
    constructor(json: JsonObject) : this(
        json["content"].asString ?: "Sorry, no answer found", json["role"].asString ?: "system"
    )
}

data class ChatCompletionChoice(
    val index: Int, val message: ChatMessage, val finishReason: String
) {
...
data class ChatCompletionRequest(
    ...
    constructor(model: String, systemContent: String) : this(
        model, arrayListOf(ChatMessage(systemContent, "system"))
    )
